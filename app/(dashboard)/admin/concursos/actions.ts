...
export async function getSalesScheduleAction() {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from('system_settings')
    .select('value')
    .eq('key', 'sales_schedule')
    .single();
...
export async function saveSalesScheduleAction(schedule: { openTime: string, closeTime: string, activeDays: number[] }) {
  try {
    const supabase = await createClient();
    ...
    await AuditService.record({
      ...
      description: 'Atualizou o cronograma global de horários de venda',
      metadata: { schedule }
    });
